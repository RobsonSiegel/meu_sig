                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-md-3 text-left ">
                                ...
                                <a id="clear-filter" class="btn btn-danger" href="{{ route('conta-receber.index') }}"><i
                    ...
                    <div class="col-md-12 mt-3">
                        <div class="table-responsive-sm">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if(!$item->status)
                                                ...
                                                <span class="text-danger"
                                                      style="font-size: 10px">{{ $item->diasAtraso() }}</span>
                                            @endif
                                        </td>
                                        <td>{{ $item->status ? __data_pt($item->data_recebimento, false) : '--' }}</td>
                                        <td>
                                            @if($item->status)
                                                <span class="btn btn-success position-relative me-lg-3 btn-sm">
                                            <i class="ri-checkbox-line"></i> Recebido
                                        </span>
                                            @else
                                                <span class="btn btn-warning position-relative me-lg-3 btn-sm">
                                        ...
                                        <td>
                                            <form action="{{ route('conta-receber.destroy', $item->id) }}" method="post"
                                                  id="form-{{$item->id}}" style="width: 220px;">
                                                ...
                                                @if(!$item->boleto)
                                                    @can('boleto_create')
                                                        <a title="Gerar boleto" class="btn btn-dark btn-sm"
                                                ...
                                                @else
                                                    @can('boleto_view')
                                                        <a title="Visualizar boleto" class="btn btn-success btn-sm"
                                                           href="{{ route('boleto.show', [$item->id]) }}">
                                                            <i class="ri-file-list-3-fill"></i>
                                                        </a>
                                                    @endcan
                                                    @if($item->boleto->enviado_email == 0)
                                                    <a title="Enviar por email" class="btn btn-primary btn-sm"
                                                       href="{{ route('boleto.enviar-email', [$item->id]) }}">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    @can('conta_receber_delete')
                                        <form action="{{ route('conta-receber.destroy-select') }}" method="post"
                                ...
                                <div class="col-md-6 text-end">
                                    @can('boleto_create')
                                        <form action="{{ route('boleto.create-several') }}" method="get"
                                              id="form-gerar-boletos">
                                            <div></div>
