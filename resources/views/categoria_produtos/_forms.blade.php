<div class="row g-2">
    ...
    @if(__isActivePlan(Auth::user()->empresa, 'Cardapio'))
        <div class="col-md-2">
            ...
            <div class="text-tooltip d-none">
                Sim se for usar esta categoria no cardápio
            </div>
        </div>
    @endif
    @if(__isActivePlan(Auth::user()->empresa, 'Delivery'))
        <div class="col-md-2">
            ...
            <div class="text-tooltip2 d-none">
                Sim se for usar esta categoria no Delivery/Marketplace
            </div>
        </div>
    ...
    @if(__isActivePlan(Auth::user()->empresa, 'Ecommerce'))
        <div class="col-md-2">
            ...
            <div class="text-tooltip3 d-none">
                Sim se for usar esta categoria no Ecommerce
            </div>
        </div>
    @endif
    ...
    @if(__isActivePlan(Auth::user()->empresa, 'Reservas'))
        <div class="col-md-2">
            ...
            <div class="text-tooltip4 d-none">
                Sim se for usar a categoria no módulo reservas
            </div>
    ...
    @if(__isActivePlan(Auth::user()->empresa, 'Chapas'))
        <div class="col-md-2">
            {!!Form::select('chapas', 'Chapa e Dobra', [0 => 'Não', 1 => 'Sim'])
            ->attrs(['class' => 'form-select tooltipp5 chapas'])
            !!}
            <div class="text-tooltip5 d-none">
                Marcar como sim se for usar esta categoria no módulo de Chapa e Dobra
            </div>
        </div>

        <div class="col-md-4 sequencia @isset($item) @else d-none @endisset">
            <label>Sequencia Padrão</label>
            <div class="input-group flex-nowrap">
                <select class="select2" name="sequencia_id" id="sequencia_id">
                    <option value="">Selecione</option>
                    @foreach($sequencias as $s)
                        <option @isset($item) @if($item->sequencia_id == $s->id) selected
                                @endif @endif value="{{ $s->id}}">{{ $s->nome }}</option>
                    @endforeach
                </select>
            </div>
        </div>
...
@section('js')
    <script type="text/javascript">
        ...
        $('body').on('change', '.chapas', function () {
            let valor = $('.chapas').val();
            if (valor == 1) {
                $('.sequencia').removeClass('d-none')
            } else {
                $('.sequencia').addClass('d-none')
            }
        });
