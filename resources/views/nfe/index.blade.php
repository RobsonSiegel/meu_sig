...
@section('content')
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="row mt-3 g-1">
                            ...
                            <div class="col-lg-4 col-12">
                                ...
                                <a id="clear-filter" class="btn btn-danger" href="{{ route('nfe.index') }}"><i
                                    ...
                                    <tr>
                                        ...
                                        @if(__isPlanoFiscal())
                                            <td width="150">
                                                @if($item->estado == 'aprovado')
                                                    <span
                                                        class="btn btn-success text-white btn-sm w-100">Aprovado</span>
                                                @elseif($item->estado == 'cancelado')
                                                    <span
                                                        class="btn btn-danger text-white btn-sm w-100">Cancelado</span>
                                                @elseif($item->estado == 'rejeitado')
                                                    <span
                                                        class="btn btn-warning text-white btn-sm w-100">Rejeitado</span>
                                                @elseif($item->estado == 'entregue')
                                                    <span
                                                        class="btn btn-success text-white btn-sm w-100">Entregue</span>
                                                @elseif($item->estado == 'inutilizada')
                                                    <span
                                                        class="btn btn-dark text-white btn-sm w-100">Inutilizada</span>
                                                @else
                                        ...
                                        <td>
                                            @if($item->pedidoEcommerce)
                                                <a title="Pedido de ecommerce" class="btn btn-sm btn-danger"
                                                   href="{{ route('pedidos-ecommerce.show', [$item->pedidoEcommerce->id]) }}">EC</a>
                                            @elseif($item->ordemServico)
                                                <a title="Ordem de serviço" class="btn btn-sm btn-primary"
                                                   href="{{ route('ordem-servico.show', [$item->ordemServico->id]) }}">OS</a>
                                            @elseif($item->pedidoMercadoLivre)
                                                <a title="Pedido mercado livre" class="btn btn-sm btn-warning"
                                                   href="{{ route('mercado-livre-pedidos.show', [$item->pedidoMercadoLivre->id]) }}">ML</a>
                                            @elseif($item->pedidoNuvemShop)
                                                <a title="Pedido nuvem shop" class="btn btn-sm btn-dark"
                                                   href="{{ route('nuvem-shop-pedidos.show', [$item->pedidoNuvemShop->pedido_id]) }}">NS</a>
                                            @else
                                                --
                                            @endif
                                        </td>
                                        <td>
                                            <form action="{{ route('nfe.destroy', $item->id) }}" method="post"
                                                  id="form-{{$item->id}}" style="width: 400px">
                                                @method('delete')
                                                @csrf
                                                {{--   Imprimir o cancelamento da NFe--}}
                                                ...
                                                @if($item->estado == 'aprovado')
                                                    {{--     Imprmir a NFe--}}
                                                    <button type="button"
                                                            onclick="imprimir('{{$item->id}}', '{{$item->numero}}')"
                                                    ...
                                                    @can('nfe_transmitir')
                                                        ...
                                                        {{--    Corrigir a NFE--}}
                                                        @if($item->estado == 'aprovado')
                                                            <button title="Corrigir NFe" type="button"
                                                ...
                                                {{--    Inutilizar Numeração da Nota --}}
                                                @if ( $item->estado == 'rejeitado')
                                                    <button title="Inutilizar NFe" type="button"
                                                            class="btn btn-primary btn-sm"
                                                            onclick="inutilizar('{{$item->id}}', '{{$item->numero}}')">
                                                ...
                                                {{--   Consultar a NFe--}}
                                                @if($item->estado == 'rejeitado')
                                                    <button title="Consultar status" type="button"
                                                ...
                                                @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                    ...
                                                    {{--  Transmitir a NFe para Sefaz   --}}
                                                    @if(__isPlanoFiscal())
                                                        @can('nfe_transmitir')
                                                            @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                                <button title="Transmitir NFe" type="button"
                                                ...
                                                {{--  Imprimir Pedido da Venda quando tiver --}}
                                                @if($item->pedidoEcommerce || $item->pedidoNuvemShop || $item->pedidoMercadoLivre)
                                                    <a class="btn btn-info btn-sm" title="Imprimir Pedido"
                                                       target="_blank"
                                                ...
                                                {{--        <button title="Consultar NFe" type="button"--}}
                                                {{--        class="btn btn-light btn-sm"--}}
                                                ...
                                                @if(__isPlanoFiscal())
                                                    @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                        @can('nfe_edit')
                                                            <a title="Alterar estado fiscal"
                                                               class="btn btn-warning btn-sm"
                                                               href="{{ route('nfe.alterar-estado', $item->id) }}">
                                                ...
                                                {{--   Informação de Entrega - DLM--}}
                                                @if($item->estado != 'entregue' && $configGeral->movimenta_estoque_entrega)
                                                    <a href="{{ route('nfe.informa-entrega', $item->id) }}"
                                                       title="Informar Entrega" type="button"
                                                ...
                                                {{--   Gerar Danfe Temporária--}}
                                                @if($item->estado == 'novo' || $item->estado == 'rejeitado')
                                                    <a class="btn btn-danger btn-sm" title="DANFE Temporária"
                                                       target="_blank"
                                                       href="{{ route('nfe.danfe-temporaria', [$item->id]) }}">
    ...
    <div class="modal fade" id="modal-cancelar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Cancelar NFe <strong class="ref-numero"></strong>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    <div class="modal fade" id="modal-corrigir" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Corrigir NFe <strong class="ref-numero"></strong>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    <div class="modal fade" id="modal-inutilizar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">

                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Inutilizar NFe <strong class="ref-numero"></strong>
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            {!!Form::text('motivo-inutilizar', 'Motivo')
                            ->required()
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Fechar</button>
                    <button type="button" id="btn-inutilizar" class="btn btn-warning">Inutilizar</button>
                </div>
            </div>
        </div>
    </div>
