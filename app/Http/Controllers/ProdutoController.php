...
class ProdutoController extends Controller
{
    ...
    protected $utilEmpresa;

    public function __construct(UploadUtil      $util, MercadoLivreUtil $utilMercadoLivre,
                                EstoqueUtil     $utilEstoque, NuvemShopUtil $utilNuvemShop, EmpresaUtil $utilEmpresa,
    ...
    public function create(Request $request)
    {
        ...
        // Alteração para inclusão do modulo de chapas.
        // Inicio Bruno
        $chapa = 0;
        ...
        return view('produtos.create',
            compact('listaCTSCSOSN', 'padroes', 'categorias', 'cardapio', 'marcas', 'delivery', 'variacoes',
                'padraoTributacao', 'ecommerce', 'configMercadoLivre', 'mercadolivre', 'configGeral', 'nuvemshop', 'reserva', 'woocommerce', 'categoriasWoocommerce', 'chapa'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $this->utilMercadoLivre->refreshToken($empresa->id);
        return view('produtos.edit',
            compact('item', 'listaCTSCSOSN', 'padroes', 'categorias', 'cardapio', 'marcas', 'delivery', 'variacoes',
                'ecommerce', 'nuvemshop', 'mercadolivre', 'woocommerce', 'configMercadoLivre', 'categoriasWoocommerce', 'chapa'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $request->merge([
                ...
                'nuvemshop' => $request->nuvemshop ? 1 : 0,
                'chapa' => $request->chapa ? 1 : 0,
            ...
            if ($request->chapa) {
                $request->merge([
                    'peso' => $request->peso_chapa ? __convert_value_bd($request->peso_chapa) : 0,
                    'comprimento' => $request->comprimento_chapa ? __convert_value_bd($request->comprimento_chapa) : '0',
                    'largura' => $request->largura_chapa ? __convert_value_bd($request->largura_chapa) : '0',
                    'espessura' => $request->espessura_chapa ? __convert_value_bd($request->espessura_chapa) : '0',
                    'diametro' => $request->diametro_chapa ? __convert_value_bd($request->diametro_chapa) : '0',
                ]);
            }
            // Fim Bruno
            ...
            $produto = DB::transaction(function () use ($request, $locais) {
                ...
                if ($request->variavel) {
                    for ($i = 0; $i < sizeof($request->valor_venda_variacao); $i++) {
                        ...
                        $dataVariacao = [
                            ...
                            'estoque_variacao' => $request->estoque_variacao[$i],
                        ...
                        if ($request->estoque_variacao[$i] && $request->estoque_variacao[$i] > 0) {
                            $estoque = $request->estoque_variacao[$i];
                        } else {
                            $estoque = 0;
                        }

                        $produtoPai = Produto::findOrFail($variacao->produto_id);
                        $produtoPai->valor_unitario = $request->valor_venda_variacao[$i];
                        $produtoPai->save();

                        if ($estoque && sizeof($locais) <= 1) {
                            $this->utilEstoque->incrementaEstoque($produto->id, $estoque, $variacao->id);
                            $transacao = Estoque::where('produto_id', $produto->id)->first();
                            $tipo = 'incremento';
                            $codigo_transacao = $transacao->id;
                            $tipo_transacao = 'alteracao_estoque';
                            $this->utilEstoque->movimentacaoProduto($produto->id, $estoque, $tipo, $codigo_transacao, $tipo_transacao, $variacao->id);
                        }
                    }
                } else {
                    if ($request->estoque_inicial && sizeof($locais) <= 1) {
                        ...
                        $codigo_transacao = $transacao->id;
                        $tipo_transacao = 'alteracao_estoque';
                        $this->utilEstoque->movimentacaoProduto($produto->id, $request->estoque_inicial, $tipo, $codigo_transacao, $tipo_transacao);
                    }
                }
                ...
                if ($request->categoria_nuvem_shop == null) {
                    $erroAlteracao = 1;
                } else {
            ...
            if ($erroAlteracao == 1) {
                session()->flash("flash_warning", "Produto atualizado com erros. Verifique as informações!");
            } else {
        ...
        try {
            ...
            if ($request->nuvemshop) {
                if ($request->categoria_nuvem_shop == null) {
                    ...
                    $request->merge([
                        'nuvem_shop_categoria' => $request->categoria_nuvem_shop]);

                }
                $resp = $this->utilNuvemShop->create($request, $item);
            }
            ...
            // Alteração para inclusão da opção de Chapas.
            // Inicio Bruno
            if ($request->chapa) {
                $request->merge([
                    'espessura' => $request->espessura ? __convert_value_bd($request->espessura) : 0,
                    'diametro' => $request->diametro ? __convert_value_bd($request->diametro) : 0,
                ]);
            }
            // Fim Bruno
            ...
            if ($request->variavel) {
                ...
                for ($i = 0; $i < sizeof($request->valor_venda_variacao); $i++) {
                    ...
                    if ($estoque > 0) {
                        $this->utilEstoque->incrementaEstoque($item->id, $estoque, $variacao->id);
                        $transacao = Estoque::where('produto_id', $item->id)->first();
                        $tipo = 'incremento';
                        $codigo_transacao = $transacao->id;
                        $tipo_transacao = 'alteracao_estoque';
                        $this->utilEstoque->movimentacaoProduto($item->id, $estoque, $tipo, $codigo_transacao, $tipo_transacao, $variacao->id);
                    }
                }
            } else {
                $variacoes = ProdutoVariacao::where('produto_id', $item->id)->get();
                foreach ($variacoes as $variacao) {
                    MovimentacaoProduto::where('produto_variacao_id', $variacao->id)->delete();
                    Estoque::where('produto_variacao_id', $variacao->id)->delete();
                    ProdutoVariacao::destroy($variacao->id);
                }
    ...
    private function __validate(Request $request)
    {
        ...
        $messages = [
            ...
            'nome.max' => 'Máximo de 60 caracteres',
                    ...
                    foreach ($row as $key => $r) {
                        ...
                        if ($r[0] != 'NOME*') {
                            ...
                            try {
                                ...
                                $localizacao = Localizacao::where('empresa_id', $request->empresa_id)->get();
                                if ($localizacao) {
                                    foreach ($localizacao as $local) {
                                        ...
                                        $this->utilEstoque->incrementaEstoque($item->id, 0, null, $local->id);

                                    }
                                    $transacao = Estoque::where('produto_id', $item->id)->get();
                                    foreach ($transacao as $t) {
                                        $tipo = 'incremento';
                                        $codigo_transacao = $t->id;
                                        $tipo_transacao = 'importacao';
                                        $this->utilEstoque->movimentacaoProduto($item->id, 0, $tipo, $codigo_transacao, $tipo_transacao, null);
                                    }
                                }
                                $cont++;
                            } catch (\Exception $e) {
    ...
    private function preparaObjeto($linha, $empresa_id)
    {
        ...
        $data = [
            ...
            'gerenciar_estoque' => $linha[18] != '' ? $linha[18] : 0,
            'perc_icms' => $linha[19] != '' ? __convert_value_bd($linha[19]) : 0,
            'perc_pis' => $linha[20] != '' ? __convert_value_bd($linha[20]) : 0,
            'perc_cofins' => $linha[21] != '' ? __convert_value_bd($linha[21]) : 0,
            'perc_ipi' => $linha[22] != '' ? __convert_value_bd($linha[22]) : 0,
            'cfop_entrada_estadual' => $linha[23],
            'cfop_entrada_outro_estado' => $linha[24],
            'referencia' => $linha[25] != null ? $linha[25] : '',
            'estoque' => $linha[26] != null ? $linha[26] : 0
    ...
    public function show($id)
    {
        ...
        // Inicio Bruno - Mostrar as quantidades pelo tipo de operação
        $total_entrada = MovimentacaoProduto::where('produto_id', $id)
            ->whereIn('tipo_transacao', ['compra', 'importacao', 'cancela_nfce', 'alteracao_estoque'])
            ->orderBy('id', 'desc')
            ->get();

        $total_saida = MovimentacaoProduto::where('produto_id', $id)
            ->whereIn('tipo_transacao', ['venda_nfe', 'venda_nfce'])
            ->get();
        // Fim Bruno
        return view('produtos.show', compact('item', 'data', 'total_saida', 'total_entrada'));
    }
    ...
    public function movimentacao($id)
    {
        ...
        if ($item->tipo_transacao == 'importacao') {
    ...
    private function criaAnuncio($request, $produto)
    {
        ...
        // Criação da tag de composição.
        if ($request->mercado_livre_composicao) {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'COMPOSITION',
                'value_name' => $request->mercado_livre_composicao
            ];
        } else {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'COMPOSITION',
        ...
        // Criação da Tag de Largura
        if ($request->largura) {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'TOWEL_WIDTH',
                'value_name' => $request->largura . ' cm'
            ];
        } else {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'TOWEL_WIDTH',
        ...
        // Criação da Tag de Comprimento
        if ($request->largura) {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'TOWEL_LENGTH',
                'value_name' => $request->comprimento . ' cm'
            ];
        } else {
            $dataMercadoLivre['attributes'][] = [
                'id' => 'TOWEL_LENGTH',
