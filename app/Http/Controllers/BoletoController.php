...
class BoletoController extends Controller
{
    ...
    public function __construct(BoletoUtil $util, MailUtil $mailUtil)
    {
        $this->util = $util;
    ...
    public function create($id){
        ...
        $nosso_numero = $contaPadrao->nosso_numero + 1;

        $contas = [];
        return view('boletos.create', compact('conta', 'contasBoleto', 'contas', 'contaPadrao', 'nosso_numero'));
    }
    ...
    public function enviarEmail($idBoleto)
    {
        $idUsuario = get_id_user();
        try {
            $response = $this->mailUtil->enviarEmailBoletos($idUsuario, $idBoleto);
            if ($response){
                $boleto = Boleto::where('conta_receber_id', $idBoleto)->first();
                $boleto->enviado_email = 1;
                $boleto->save();
