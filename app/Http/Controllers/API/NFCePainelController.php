...
class NFCePainelController extends Controller
{
    ...
    public function cancelar(Request $request)
    {
        ...
        if ($nfce != null) {
            ...
            if (!isset($doc['erro'])) {
                ...
                // Alteração para quando cancelar a venda, estornar o estoque
                // Inicio Bruno
                $itens = $nfce->itens;
                foreach ($itens as $i){
                    $item = ItemNfce::findOrFail($i->id);

                    // Adiciona novamente a quantidade ao estoque
                    $this->util->incrementaEstoque($item->produto_id, $item->quantidade, null, $nfce->local_id );

                    // Cria movimentação de estorno de cancelado
                    $tipo = 'incremento';
                    $codigo_transacao = $nfce->id;
                    $tipo_transacao = 'cancela_nfce';
                    $this->util->movimentacaoProduto($item->produto_id,$item->quantidade, $tipo, $codigo_transacao, $tipo_transacao, null);
                }
                // Fim Bruno
    ...
    public function inutilizar(Request $request)
    {
        ...
        if ($item != null) {
            ...
            $consulta = $nfce_service->inutilizar($item);
