...
class ProdutoController extends Controller
{
    public function pesquisa(Request $request)
    {
        ...
        $data = Produto::orderBy('nome', 'desc')
            ...
            ->orWhere('referencia', 'LIKE', "%$request->pesquisa%") // Adicionando a busca pela referência fora das condições
    ...
    //Função usada para retornar os produtos para a requisição do PDV Delphi.
    // Inicio Bruno
    public function buscaProdutoPDV(Request $request)
    {
        $data = Produto::where('produtos.empresa_id', $request->empresa_id)
            ->select(
                ...
                'produtos.perc_ipi',
                'produtos.cfop_estadual',
                'produtos.cfop_outro_estado',
                'estoques.quantidade',
                'estoques.local_id'
            )
            ->leftJoin('estoques', function ($join) use ($request) {
                $join->on('estoques.produto_id', '=', 'produtos.id')
                    ->where('estoques.local_id', '=', $request->local_id);
            })
    ...
    public function getVariacoes($idProduto)
    {
        $variacoes = ProdutoVariacao::where('produto_id', $idProduto)
    ...
    public function getValorVariacao($idVariacao){
        $variacao = ProdutoVariacao::findOrFail($idVariacao);
    ...
    public function pesquisaCodigoBarra(Request $request)
    {
        $local_id = $request->local_id;
        $produto = Produto::where('codigo_barras', $request->barcode)
    ...
    public function atualizaEstoque(Request $request)
    {
        $local_id = $request->local_id;
        $produto = Produto::where('codigo_barras', $request->barcode)
            ->where('empresa_id', $request->empresa_id)->first();
        ...
        $estoque->quantidade = $request->nova_quantidade;
        $estoque->save();
        ...
        MovimentacaoProduto::create([
            ...
            'tipo_transacao' => 'contagem_estoque',
        ]);

        return response()->json($produto, 201);
    }

    public function getNaturezaOperacao(Request $request){
        $produto = Produto::findOrFail($request->produto);
        $cliente = Cliente::findOrFail($request->cliente);
        $empresa = Empresa::findOrFail(request()->empresa_id);
        $estadoCliente = $cliente->cidade->uf;
        $estadoEmpresa = $empresa->cidade->uf;
        $cfop = '';

        if ($estadoCliente == $estadoEmpresa){
            $cfop = $produto->cfop_estadual;
        }else{
            $cfop = $produto->cfop_outro_estado;
        }
        return response()->json($cfop, 200);
