...
class EmpresaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            DB::transaction(function () use ($request) {
                ...
                /* Criação das informações fixas do módulo Chapas */
                /* Inicio Bruno*/
                $this->empresaUtil->createSetores($empresa->id);
                $this->empresaUtil->createMaquinas($empresa->id);
                $this->empresaUtil->createSequenciaPadrao($empresa->id);
    ...
    private function deleteRegistros($empresa_id){
        ...
        $local = Localizacao::where('empresa_id', $empresa_id)->get();
        foreach($local as $us){
            $usuarioLocalizacao = UsuarioLocalizacao::where('localizacao_id', $us->id)->get();
            foreach($usuarioLocalizacao as $u){
                $u->delete();
            }
            $us->delete();
        }

        Localizacao::where('empresa_id', $empresa_id)->delete();

        $caixas = Caixa::where('empresa_id', $empresa_id)->get();

        Caixa::where('empresa_id', $empresa_id)->delete();

        $cardapios = ConfiguracaoCardapio::where('empresa_id', $empresa_id)->delete();
        $marketplaces = MarketPlaceConfig::where('empresa_id', $empresa_id)->delete();


        $tickets = \App\Models\Ticket::where('empresa_id', $empresa_id)->get();
        foreach($tickets as $t){
            $t->mensagens()->delete();
            $t->delete();
        }

        $variacao = \App\Models\VariacaoModelo::where('empresa_id', $empresa_id)->get();
        foreach ($variacao as $v){
            $v->itens()->delete();
            $v->delete();
        }

        $ordens = \App\Models\OrdemServico::where('empresa_id', $empresa_id)->get();
        foreach ($ordens as $o){
            $o->relatorios()->delete();
            $o->servicos()->delete();
            $o->delete();
        ...
        \App\Models\PlanoConta::where('empresa_id', $empresa_id)->delete();
        ...
        \App\Models\PreVenda::where('empresa_id', $empresa_id)->delete();
        \App\Models\Transportadora::where('empresa_id',$empresa_id)->delete();
        \App\Models\Servico::where('empresa_id', $empresa_id)->delete();
        \App\Models\ProdutoFornecedor::where('empresa_id', $empresa_id)->delete();
        \App\Models\Pagamento::where('empresa_id', $empresa_id)->delete();
        \App\Models\Funcionario::where('empresa_id', $empresa_id)->delete();
