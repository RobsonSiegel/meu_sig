...
class NfeController extends Controller
{
    ...
    public function __construct(EstoqueUtil $util, MailUtil $mailUtil)
    {
        $this->util = $util;
    ...
    public function index(Request $request)
    {
        ...
        return view('nfe.index', compact('data', 'contigencia', 'configGeral'));
    }
    ...
    public function create()
    {
        ...
        return view(
            'nfe.create',
            compact('clientes', 'transportadoras', 'cidades', 'naturezas', 'numeroNfe', 'empresa', 'caixa')
        );
    }
                ...
                for ($i = 0; $i < sizeof($request->produto_id); $i++) {
                    ...
                    if ($request->tipo_pagamento) {
                        ...
                        if ($request->tpNF == 1) {
                            ...
                            //Feito alteração para gerar financeiro só quando a opção de Movimentar Estoque na Entrega estiver desmarcada.
                            if (!$configGeral->movimenta_estoque_entrega) {
                                for ($i = 0; $i < sizeof($tipoPagamento); $i++) {
                                    if ($request->estado == 'recebido') {
    ...
    private
    function cadastrarCliente($request)
    {
    ...
    private
    function atualizaCliente($request)
    {
    ...
    private
    function cadastrarFornecedor($request)
    {
    ...
    private
    function atualizaFornecedor($request)
    {
    ...
    private
    function cadastrarTransportadora($request)
    {
    ...
    private
    function atualizaTransportadora($request)
    {
        if ($request->razao_social_transp) {
    ...
    function destroy($id)
    {
        ...
        try {
            foreach ($item->itens as $i) {
                if ($item->tpNF == 1) { //Alteração feita para alterar estoque apenas das NFes de Vendas.
    ...
    public
    function inutilizar(Request $request)
    {
        $start_date = $request->get('start_date');
        $end_date = $request->get('end_date');
    ...
    public
    function inutilStore(Request $request)
    {
        $request->merge([
            'estado' => 'novo',
            'modelo' => '55'
    ...
    public
    function storeEstado(Request $request, $id)
    {
        $item = Nfe::findOrFail($id);
        try {
    ...
    public
    function informaEntrega($id)
    {
        $venda = Nfe::findOrFail($id);
        $produtos = [];
        foreach ($venda->itens as $i) {
            array_push($produtos, $i);
        }
        return view('nfe.info_entrega', compact('produtos', 'venda'));
    }

    public
    function setarInfoEntrega(Request $request)
    {
        $data = Nfe::where('id', $request->venda_id)->first();

        for ($i = 0; $i < sizeof($request->produto_id); $i++) {
            $produto_id = (int)$request->produto_id[$i];
            $this->util->reduzEstoque($produto_id, __convert_value_bd($request->quantidade[$i]),
                null, $data->local_id);
            $tipo = 'reducao';
        ...
        session()->flash("flash_success", "Dados de entrega de produtos salvo com sucesso!");
        return redirect()->route('nfe.index');
    }

    public function enviarEmail($idNota)
    {
        $idUsuario = get_id_user();
        try {
            $response = $this->mailUtil->enviarEmailNFe($idUsuario, $idNota);
            if ($response) {
            ...
            session()->flash('flash_error', 'Erro ao enviar email: ' . $e->getMessage());
        } finally {
            return redirect()->back();
        }
    }
