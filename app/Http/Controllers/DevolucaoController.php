...
//                        'vl_base_icms' => __convert_value_bd($request->vl_base_icms[$i]),
    ...
    private function atualizaTransportadora($request)
    {
        ...
        session()->flash('flash_error', 'Transportada não encontrada');
        return false;
    }
            ...
            DB::transaction(function () use ($request, $nfe) {
                ...
                for ($i = 0; $i < sizeof($request->produto_id); $i++) {
                    ...
                    ItemNfe::create([
                        ...
                        'perc_pis' => $request->perc_icms[$i] ? __convert_value_bd($request->perc_pis[$i]) : 0,
                        'perc_cofins' => $request->perc_icms[$i] ? __convert_value_bd($request->perc_cofins[$i]) : 0,
                        'perc_ipi' => $request->perc_icms[$i] ? __convert_value_bd($request->perc_ipi[$i]) : 0,
                        'cst_csosn' => $request->cst_csosn[$i],
                        'cst_pis' => $request->cst_pis[$i],
                        'cst_cofins' => $request->cst_cofins[$i],
                        'cst_ipi' => $request->cst_ipi[$i],
                        ...
                        'cfop' => $request->cfop[$i],
                        'ncm' => $request->ncm[$i],
                        'codigo_beneficio_fiscal' => $request->codigo_beneficio_fiscal[$i] ? $request->codigo_beneficio_fiscal[$i] : ''
                    ]);
