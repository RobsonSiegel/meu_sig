...
class ConfigGeralController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            //Inicio Bruno - Ticket:
            $request->merge([
                'movimenta_estoque_entrega' => $request->movimenta_estoque_entrega ? 1: 0
            ]);
            //Fim Bruno

            //Inicio Bruno - Ticket: 5125
            $request->merge([
                'mostra_campos_extra_fechamento_caixa' => $request->mostra_campos_extra_fechamento_caixa ? 1: 0]);
            //Fim Bruno

            //Inicio Bruno - Ticket: 5127
            $request->merge([
                'usa_caixa_cego' => $request->usa_caixa_cego ? 1: 0]);
            //Fim Bruno

            //Inicio Bruno - Ticket: 5130
            $request->merge([
                'tipo_imp_fechamento_caixa' => $request->tipo_imp_fechamento_caixa ? $request->tipo_imp_fechamento_caixa: '01']);
            //Fim Bruno

            //Inicio Bruno - Ticket: 5129
            $request->merge([
                'percentual_maximo_desconto' => $request->percentual_maximo_desconto ? $request->percentual_maximo_desconto: 0]);
            //Fim Bruno
