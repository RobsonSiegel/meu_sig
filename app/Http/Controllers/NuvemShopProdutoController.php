...
class NuvemShopProdutoController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($search != ""){
            $produtos = (array)$api->get("products?q='".$search."'&per_page=30");
        }else{
            $produtos = (array)$api->get("products?page=".$page."&per_page=" .env("PAGINACAO"));
        }
        $data = $produtos['body'];
    ...
    try{
        ...
        if(sizeof($produto->variants) == 1){
            ...
            if($request->estoque){
                ...
                $this->utilEstoque->movimentacaoProduto($item->id, $request->estoque, $tipo, $codigo_transacao, $tipo_transacao);
            }
