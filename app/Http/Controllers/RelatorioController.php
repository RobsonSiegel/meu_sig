...
class RelatorioController extends Controller
{
    ...
    public function produtosNegativos(Request $request)
    {
        ...
        $data = Produto::select(
            ...
            'estoques.quantidade as qtd_estoque',
            'produto_localizacaos.localizacao_id as local_id',
            'localizacaos.descricao as desc_localizacao',
            DB::raw('SUM(CASE WHEN nves.tpNF = 0 AND nves.estado NOT IN ("recebido", "cancelado") THEN item_nves.quantidade ELSE 0 END) as qtd_compra'),
            DB::raw('SUM(CASE WHEN nves.tpNF = 1 AND nves.estado NOT IN ("entregue", "cancelado") THEN item_nves.quantidade ELSE 0 END) as qtd_venda')
        )
            ...
            ->when($local_id, function ($query) use ($local_id) {
                return $query->where('produto_localizacaos.localizacao_id', $local_id);
            })
            ->when(!$local_id, function ($query) use ($locais) {
                return $query->whereIn('produto_localizacaos.localizacao_id', $locais);
            })
            ->leftJoin('estoques', 'estoques.produto_id', '=', 'produtos.id')
            ->leftJoin('produto_localizacaos', 'produto_localizacaos.produto_id', '=', 'produtos.id')
            ->leftJoin('localizacaos', 'localizacaos.id', '=', 'produto_localizacaos.localizacao_id')
            ->leftJoin('item_nves', 'item_nves.produto_id', '=', 'produtos.id')
            ->leftJoin('nves', 'nves.id', '=', 'item_nves.nfe_id')
            ->groupBy('produtos.id')
            ->get();

        $p = view('relatorios/produtos_negativos', compact('data'))
            ->with('title', 'Relatório de Produtos Negativos');
        $domPdf = new Dompdf(["enable_remote" => true]);
        $domPdf->loadHtml($p);
