...
class PaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Definir os planos específicos para empresas
        $planosEspecificos = [
            'FAYRA' => 6,
            'PEGAPACK' => 9
        ];

        // Verificar se a empresa possui um plano específico, senão aplica o padrão
        $planoId = $planosEspecificos[$empresa->nome_fantasia] ?? null;

        if ($planoId) {
            $planos = Plano::where('id', $planoId)->get();
        } else {
