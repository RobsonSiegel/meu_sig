...
class MovimentacaoProduto extends Model
{
    ...
    public function tipoTransacao(){
        if($this->tipo_transacao == 'venda_nfe'){
            ...
            return 'Compra';
        }else if($this->tipo_transacao == 'importacao') {
            return 'Importação';
        }else if($this->tipo_transacao == 'cancela_nfce'){
                return 'Cancelamento de NFCe';
        } else{
            return 'Alteração de estoque';
        }
    }

    public function getLocalMovimentacao($id){
        if ($id != 1){
            $estoque = Estoque::where('id', $id)->first();
            if ($estoque){
                $localizacao = Localizacao::where('id', $estoque->local_id)->first();
