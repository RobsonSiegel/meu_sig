    ...
    public function gerarXml($item){
        ...
        foreach ($item->itens as $itemCont => $i) {
            ...
            if ($stdEmit->CRT == 1 || $stdEmit->CRT == 4) {
                ...
                $stdICMS->modBC = 0;
                $stdICMS->vBC = $this->format($i->valor_unitario * $i->quantidade - $stdProd->vDesc);
                ...
                if($i->perc_red_bc > 0){
                    ...
                    $valorBase = $stdICMS->vBC;
                    $valorICMS = $stdICMS->vICMS;
                    $VBC += $stdICMS->vBC;
        ...
        if($item->transportadora){
            ...
            $std->xEnder = $item->transportadora->logradouro;
            $std->xMun = $item->transportadora->cidade->nome;
            $std->UF = $item->transportadora->cidade->uf;

            $cnpj_cpf = preg_replace('/[^0-9]/', '', $item->transportadora->cnpj_cpf);
