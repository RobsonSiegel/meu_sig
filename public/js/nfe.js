let buscarComprasClicked = false;
...
function buscarNumeroCompras() {
    buscarComprasClicked = true; // Marcar que o botão de buscar compras foi clicado
    let produtoIds = [];
    $('#produtos .codigo_produto').each(function() {
        produtoIds.push($(this).data('codigo_produto'));
    });

    return new Promise((resolve, reject) => {
        $.ajax({
            url: path_url + "api/compras/buscar-numero-compra",
            type: "POST",
            data: JSON.stringify({ produto_ids: produtoIds }), // Enviando como JSON
            ...
            success: function(response) {
                ...
                response.forEach(function(item) {
                    $('#produtos .codigo_produto').each(function() {
                        if ($(this).data('codigo_produto') === item.produto_id) {
                            promises.push(validaItensCompra(item.numero_compra));
                            $(this).closest('tr').find('.numero_compra').val(item.numero_compra);
                        }
                    });
                });
                Promise.all(promises).then(results => {
                    const hasDivergence = results.some(result => result === true);
                    resolve(hasDivergence);
                });
            },
...
function validaItensCompra(numeroCompra) {
    return new Promise((resolve, reject) => {
        let hasDivergence = false;
        if (numeroCompra > 0) {
            $.get(path_url + "api/compras/find/" + numeroCompra)
                .done((res) => {
                    res.forEach(function(item) {
                        $('#produtos .codigo_produto').each(function() {
                            if ($(this).data('codigo_produto') === item.produto_id) {
                                let $row = $(this).closest('tr');
                                let unidadeNota = $row.find('.unidade').data('unidade');
                                let quantidadeNota = parseFloat($row.find('.qtd').val().replace(',', '.')).toFixed(2);
                                let valorUnitarioNota = parseFloat($row.find('.valor_unit').val().replace(',', '.')).toFixed(2);

                                if (unidadeNota !== item.unidade_medida || quantidadeNota !== parseFloat(item.quantidade).toFixed(2) || valorUnitarioNota !== parseFloat(item.valor_unitario).toFixed(2)) {
                                    swal("Campos divergentes", 'Campos do item da nota divergente do item da compra encontrada! Verifique: Unidade de Medida / Quantidade do Item / Valor Unitário', "warning");
                                    hasDivergence = true;
                                }
                            }
                        });
...
$(document).on("change", ".produto_id", function () {
    ...
    if (product_id) {
        ...
        $cbenef = $ncm.closest('td').next().find('input');
        $cst_csosn = $cbenef.closest('td').next().find('select');
        ...
        $.get(path_url + "api/produtos/find",
            ...
            .done((e) => {
                ...
                $ncm.val(e.ncm)
                $cbenef.val(e.codigo_beneficio_fiscal)
...
function buscarVariacoes(produto_id) {
    $.get(path_url + "api/variacoes/find", {produto_id: produto_id})
        .done((res) => {
            $('#modal_variacao .modals-body').html(res)
...
$('body').on('blur', '.valor_unit', function () {
    ...
    // Valor Base ICMS
    // const $valor_base_icms = $(this).closest('tr').find('.valor_base_icms');
    // $valor_base_icms.val($sub.val());

    // Valor Base PIS
    // const $valor_base_pis = $(this).closest('tr').find('.valor_base_pis');
    // $valor_base_pis.val($sub.val());

    // Valor Base Cofins
    // const $valor_base_cofins = $(this).closest('tr').find('.valor_base_cofins');
    // $valor_base_cofins.val($sub.val());

    // Valor Base IPI
    // const $valor_base_ipi = $(this).closest('tr').find('.valor_base_ipi');
    // $valor_base_ipi.val($sub.val());
})

// $('body').on('blur', '.perc_icms', function () {
//     $base = $(this).closest('td').prev().find('input');
//     $valor = $(this).closest('td').next().find('input');
//     let percentual = $(this).val();
//     let base = convertMoedaToFloat($base.val())
//     $valor.val(convertFloatToMoeda(base * percentual / 100))
...
function validacao(event) {
    event.preventDefault(); // Previne o comportamento padrão do botão de submissão

    if (buscarComprasClicked) {
        buscarNumeroCompras().then((divergencia) => {
            if (divergencia) {
                swal({
                    title: "Divergências encontradas",
                    text: "Foram encontradas divergências entre os itens da nota e as compras. Deseja realmente salvar a nota com essas divergências?",
                    icon: "warning",
                    buttons: {
                        cancel: "Cancelar",
                        confirm: {
                    ...
                    if (willSave) {
                        $('.btn-salvar-nfe').click(); // Submete o formulário
                    } else {
                        // Usuário cancela salvar com divergências
                        console.log('Usuário cancelou salvar com divergências.');
                        // Exibe as divergências automaticamente
                        $('#btn-buscar-compra').click();
                    }
                });
            } else {
                $('.btn-salvar-nfe').click(); // Submete o formulário
        ...
        $('.btn-salvar-nfe').click(); // Submete o formulário diretamente se o botão de buscar compras não foi clicado
    }
}
