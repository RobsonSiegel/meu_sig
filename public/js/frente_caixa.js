...
function calcTotal() {
    ...
    setTimeout(() => {
        ...
        $(".total-venda-modals").html("R$ " + convertFloatToMoeda(total + VALORACRESCIMO - DESCONTO));
...
$(".btn-modals-multiplo").on("click", (event) => {
...
function buscaConfigDesconto() {
    PERCENTUALMAXDESCONTO = 0;
    let empresa_id = $('#empresa_id').val()
    $.get(path_url + "api/busca-config-geral/" + empresa_id)
        .done((e) => {
            if (e) {
                if (e.percentual_maximo_desconto > 0) {
                    PERCENTUALMAXDESCONTO = e.percentual_maximo_desconto;
                }
            }
        })
        .fail((e) => {
...
$("#inp-tipo_pagamento").change(() => {
    ...
    if (tipo === "01") {
        ...
        $(".div-vencimento").addClass('d-none');
        $(".div-nao-fiscal").removeClass('d-none');

    } else {
        $("#inp-valor_recebido").attr("disabled", "true");
        $(".div-troco").addClass('d-none');
        $(".div-nao-fiscal").addClass('d-none');
...
$(".modals-funcioario select").each(function () {
...
//     emitirNfce = false
//     // if ($("#form-pdv-update")) {
//     //     console.log('Alterando a venda no Form PDV');
...
$("#form-pdv").on("submit", function (e) {
    ...
    console.log(">>>>>>>> salvando Venda ", json);
