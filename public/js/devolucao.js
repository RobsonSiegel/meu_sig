...
$(document).on("change", ".produto_id", function () {
    ...
    if (product_id) {
        ...
        // $valor_base_icms = $sub.closest('td').next().find('input');
        $perc_icms = $valor_base_icms.closest('td').next().find('input');
        // $valor_icms = $perc_icms.closest('td').next().find('input');

        // $valor_base_pis = $valor_icms.closest('td').next().find('input');
        $perc_pis = $perc_icms.closest('td').next().find('input');
        // $valor_pis = $perc_pis.closest('td').next().find('input');

        // $valor_base_cofins = $valor_pis.closest('td').next().find('input');
        $perc_cofins = $perc_pis.closest('td').next().find('input');
        // $valor_cofins = $perc_cofins.closest('td').next().find('input');

        // $valor_base_ipi = $valor_cofins.closest('td').next().find('input');
        ...
        $.get(path_url + "api/produtos/find",
            ...
            .done((e) => {
                ...
                // $valor_icms.val(convertFloatToMoeda( $valor_base_icms * $perc_icms / 100))
                // $valor_base_pis.val($sub.val())
                $perc_pis.val(e.perc_pis)
                // $valor_pis.val(convertFloatToMoeda($valor_base_pis * $perc_pis / 100))
                // $valor_base_cofins.val($sub.val())
                $perc_cofins.val(e.perc_cofins)
                // $valor_cofins.val(convertFloatToMoeda($valor_base_cofins * $perc_cofins / 100))
                // $valor_base_ipi.val($sub.val())
                $perc_ipi.val(e.perc_ipi)
                // $valor_ipi.val(convertFloatToMoeda($valor_base_ipi * $perc_ipi / 100))
