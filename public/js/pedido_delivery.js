...
$('.btn-novo-endereco').click(() => {
    $('#modals-enderecos').modal('hide')
    $('#modals-novo-endereco').modal('show')
})
...
function changePizza(produto){
    $('#modals-pizza').modal('show')
    SABORESSELECIONADOS = []
    $('#modals-pizza .modals-body .pizzas').html('')
...
function buscasPizzas(tamanho_id){
    ...
    .done((success) => {
        $('#modals-pizza .modals-body .pizzas').html(success)
        calculaValorPizza()
    })
    .fail((err) => {
        console.log(err)
