...
function changePadraoTributacao(){
...
function changeChapa() {
    let check = $('#inp-chapa').is(':checked')
    if (check) {
        $('.div-chapa').removeClass('d-none')
        $('.inp-chapa').attr('required', 1)
    } else {
        $('.div-chapa').addClass('d-none')
        $('.inp-chapa').removeAttr('required')
    }
}
...
$(document).on("change", "#inp-padrao_id", function() {
    ...
    if (padrao) {
        ...
        .done((result) => {
            ...
            var newOption = new Option(result._ncm.descricao, result._ncm.codigo, 1, true);
            $('#inp-ncm').append(newOption);

            console.log('Novo NCM: ' + result._ncm.codigo);
...
function changeNuvemShop() {
    ...
    if (check) {
        /* Alteração feita para pegar corretamente o valor unitário do Item*/
         let valor = $('#inp-valor_unitario').val();
        $('#inp-nuvem_shop_valor').val(valor);
...
$(document).on("blur", "#inp-valor_unitario", function () {
       $('#inp-valor_ecommerce').val($(this).val())
    $('#inp-mercado_livre_valor').val($(this).val())
